...
Snake.prototype.init=function(){
    ...
    snakeBody1.create();
    this.pos.push=([1,0]);//把蛇身体1的坐标也存起来
    //创建蛇身体2
    var snakeBody2=new Square(0,0,'snakeBody');
    snakeBody2.create();
    this.tail=snakeBody2;//把蛇尾的信息存起来
    this.pos.push=([0,0]);//把蛇身体1的坐标也存起来
...
Snake.prototype.strategies={
    move:function(format){//这个参数用于决定要不要删除最后一个方块（蛇尾）。当传了这个参数后就表示要做的事情是吃。
        ...
        //蛇身上每一个方块的坐标也要更新
        this.pos.splice(0,0,[0,0,this.head.x/sw+this.direction.x,this.head.y/sh+this.direction.y])
...
function createFood(){
    ...
    var include=true;//循环跳出的条件，true表示食物的坐标在蛇身上,false表示食物的坐标不在蛇身上（不循环了）
    // while(include){
...
Game.prototype.init=function(){
    ...
    document.onkeydown=function(ev){
        if(ev.which==65 && snake.direction!=snake.directionNum.right){//用户按下左键的时候，这条蛇不能是正往右走
